...
import { Download, BarChart3, PieChartIcon, TrendingUp, Users } from "lucide-react"
...
export default function ReportsPage() {
  ...
  const filteredLeads = useMemo(() => {
    ...
    let filtered = user.role === "admin" ? leads : leads.filter((lead) => lead.createdById === user.id)
  ...
  const statusData = useMemo(() => {
    ...
    return [
      ...
      { name: "Interview Scheduled", value: statusCounts.interview_scheduled, color: "#3B82F6" },
      { name: "Test Assigned", value: statusCounts.test_assigned, color: "#F59E0B" },
  ...
  const userPerformance = useMemo(() => {
    ...
    const performance = users
      ...
      .map((u) => {
        const userLeads = filteredLeads.filter((lead) => lead.createdById === u.id)
        return {
          id: u.id,
          name: u.name,
          role: u.role,
          ...
          completedLeads: userLeads.filter((l) => l.status === "completed").length,
          conversionRate:
            userLeads.length > 0
              ? Math.round((userLeads.filter((l) => l.status === "completed").length / userLeads.length) * 100)
              : 0,
        }
      })
  ...
  const exportToCSV = () => {
    const csvData = [
      ["Client Name", "Job Description", "Source", "Status", "Assigned Developer", "Created By", "Created Date"],
  ...
  if (!hasPermission(user.role, "read", "reports")) {
    return (
      <MainLayout>
    ...
    <MainLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <Button onClick={exportToCSV} className="bg-validiz-brown hover:bg-validiz-brown/90">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-validiz-brown">{filteredLeads.length}</div>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                {filteredLeads.length > 0
                  ? Math.round(
                      (filteredLeads.filter((l) => l.status === "completed").length / filteredLeads.length) * 100,
                    )
                  : 0}
                % conversion rate
              </p>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-blue-600">
                {filteredLeads.filter((l) => l.status === "interview_scheduled" || l.status === "test_assigned").length}
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card>
            <CardHeader>
              <CardTitle className="text-validiz-brown">Leads by Status</CardTitle>
            </CardHeader>
            <CardContent>
                ...
                <TableBody>
                  {userPerformance.map((user) => (
                    <TableRow key={user.id} className="hover:bg-gray-50">
                      ...
                      <TableCell className="font-semibold">{user.totalLeads}</TableCell>
                      <TableCell>{user.newLeads}</TableCell>
                      <TableCell className="text-green-600 font-semibold">{user.completedLeads}</TableCell>
                      <TableCell>
