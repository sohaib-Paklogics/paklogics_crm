...
export default function DashboardPage() {
  ...
  const stats = {
    ...
    interviewScheduled: userLeads.filter((lead) => lead.status === "interview_scheduled").length,
    ...
    <MainLayout>
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-gray-600 mt-2">Welcome back to your Validiz CRM dashboard</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-validiz-brown">{stats.totalLeads}</div>
              <p className="text-xs text-muted-foreground">
                {user.role === "admin" ? "All leads" : user.role === "bd" ? "Your leads" : "Assigned to you"}
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-validiz-mustard">{stats.newLeads}</div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-blue-600">{stats.interviewScheduled}</div>
              ...
              {userLeads.slice(0, 5).map((lead) => (
                <div key={lead.id} className="flex items-center justify-between p-4 border rounded-lg">
                  <div className="flex-1">
                    <h4 className="font-medium text-validiz-brown">{lead.clientName}</h4>
