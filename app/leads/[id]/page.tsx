...
import { ArrowLeft, Edit, Save, X, Upload, Download, MessageCircle, FileText, Clock } from "lucide-react"
import Link from "next/link"
import type { Lead, ChatMessage, Note, HistoryEntry, Attachment } from "@/lib/types"
...
export default function LeadDetailPage() {
  ...
  const [chatMessages] = useState<ChatMessage[]>([
    {
      id: "1",
      leadId: leadId,
      message: "Initial contact made with the client. They seem very interested in our services.",
      userId: "2",
    ...
    },
    {
      id: "2",
      leadId: leadId,
      message: "I've reviewed the job requirements. This looks like a perfect match for my experience.",
      userId: "3",
  ...
  const canEdit =
    hasPermission(user.role, "update", "leads") && (user.role === "admin" || selectedLead.createdById === user.id)
  const canViewNotes = hasPermission(user.role, "read", "notes")
  const canAddNotes = hasPermission(user.role, "create", "notes")
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-validiz-brown">{selectedLead.clientName}</h1>
              <div className="flex items-center space-x-2 mt-1">
                <Badge className={selectedLead.status === "new" ? "bg-validiz-mustard text-validiz-brown" : ""}>
                  {selectedLead.status.replace("_", " ")}
                </Badge>
                <span className="text-gray-500">•</span>
                <span className="text-gray-600 capitalize">{selectedLead.source.replace("_", " ")}</span>
              </div>
            </div>
          </div>
          {canEdit && (
            <div className="flex space-x-2">
              {isEditing ? (
                <>
                  ...
                  <Button onClick={handleSave} className="bg-validiz-brown hover:bg-validiz-brown/90">
                    <Save className="mr-2 h-4 w-4" />
                    Save Changes
                  </Button>
                </>
              ) : (
                <Button onClick={() => setIsEditing(true)} className="bg-validiz-brown hover:bg-validiz-brown/90">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="details" className="space-y-6">
            <Card>
              <CardHeader>
                <CardTitle className="text-validiz-brown">Lead Details</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      id="clientName"
                      value={isEditing ? editData.clientName || "" : selectedLead.clientName}
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      value={isEditing ? editData.source || selectedLead.source : selectedLead.source}
                      onValueChange={(value) => setEditData({ ...editData, source: value as any })}
                      disabled={!isEditing}
                    >
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    id="jobDescription"
                    value={isEditing ? editData.jobDescription || "" : selectedLead.jobDescription}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Select
                      value={isEditing ? editData.status || selectedLead.status : selectedLead.status}
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setEditData({ ...editData, assignedDeveloperId: value === "unassigned" ? undefined : value })
                      }
                      disabled={!isEditing}
                    >
                      <SelectTrigger className="focus:ring-validiz-mustard focus:border-validiz-mustard">
          ...
          <TabsContent value="attachments" className="space-y-6">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <CardTitle className="text-validiz-brown">Attachments</CardTitle>
                <Button size="sm" className="bg-validiz-brown hover:bg-validiz-brown/90">
          ...
          <TabsContent value="history" className="space-y-6">
            <Card>
              <CardHeader>
                <CardTitle className="text-validiz-brown">Activity History</CardTitle>
              </CardHeader>
              <CardContent>
          ...
          <CardContent>
            ...
            <div className="flex space-x-2">
              <Input
                ...
                className="focus:ring-validiz-mustard focus:border-validiz-mustard"
              />
              <Button onClick={handleSendMessage} className="bg-validiz-brown hover:bg-validiz-brown/90">
                Send
              </Button>
            </div>
          </CardContent>
                ...
                {canAddNotes && (
                  <div className="space-y-2">
                    <Textarea
                      ...
                      className="focus:ring-validiz-mustard focus:border-validiz-mustard"
                    />
                    <Button onClick={handleAddNote} size="sm" className="bg-validiz-brown hover:bg-validiz-brown/90">
                      Add Note
                    </Button>
                  </div>
                )}
