...
import { FileText, Calendar, BarChart3, Settings, User, LogOut, Menu, X, Kanban, Home } from "lucide-react"

const navigation = [
  { name: "Dashboard", href: "/dashboard", icon: Home, permission: null },
  { name: "Leads", href: "/leads", icon: FileText, permission: { action: "read", resource: "leads" } },
  { name: "Kanban", href: "/kanban", icon: Kanban, permission: { action: "read", resource: "leads" } },
  { name: "Calendar", href: "/calendar", icon: Calendar, permission: { action: "read", resource: "calendar" } },
  { name: "Reports", href: "/reports", icon: BarChart3, permission: { action: "read", resource: "reports" } },
  { name: "Admin Panel", href: "/admin", icon: Settings, permission: { action: "read", resource: "users" } },
]
...
export function Sidebar() {
  ...
  const filteredNavigation = navigation.filter((item) => {
    ...
    return hasPermission(user.role, item.permission.action, item.permission.resource)
  })
