...
type VariantKey = "success" | "destructive" | "warning" | "info" | "default" | "brown";
...
function classifyStatus(token: string): {
  ...
  // Hard map the three pills from your design
  if (token === "new") {
    return { label: "New", icon: Clock, variant: "info" }; // blue
  }
  if (token === "follow_up" || token === "followup") {
    return { label: "Follow Up", icon: Clock, variant: "brown" }; // brown
  }
  if (token === "completed" || token === "done") {
    return { label: "Completed", icon: CheckCircle, variant: "success" }; // green
  }

  // --- generic heuristic (kept from your version) ---
  const SUCCESS = new Set(["success","succeeded","synced","ok","healthy","passing","passed","completed","done","resolved","approved","active","live","up","ready","verified","accepted","published"]);
  const DANGER  = new Set(["failed","failure","error","errored","reject","rejected","declined","fatal","critical","down","offline","terminated","canceled","cancelled","expired","revoked","blocked","deleted","archived","closed"]);
  const WARNING = new Set(["pending","waiting","on_hold","hold","paused","pause","investigating","needs_attention","delayed","stuck","review","qa","code_review","verification","escalated","degraded"]);
  const INFO    = new Set(["processing","in_progress","running","building","deploying","syncing","starting","initializing","provisioning","migrating","updating","rolling_out","queued","scheduling","scheduled","active","open"]);
  const STAGE_TODO = new Set(["new","todo","to_do","backlog"]);
  const STAGE_DOING = new Set(["doing","in_progress","wip"]);
  const STAGE_REVIEW = new Set(["review","qa","testing","code_review","verify","verification"]);
  const STAGE_DONE = new Set(["done","completed","shipped","delivered"]);

  if (SUCCESS.has(token) || STAGE_DONE.has(token)) return { label: prettyLabel(token), icon: CheckCircle, variant: "success" };
  if (DANGER.has(token)) return { label: prettyLabel(token), icon: XCircle, variant: "destructive" };
  if (WARNING.has(token) || STAGE_REVIEW.has(token)) {
    const icon = token.includes("pause") || token.includes("hold") ? PauseCircle : AlertCircle;
    return { label: prettyLabel(token), icon, variant: "warning" };
  }
  if (INFO.has(token) || STAGE_DOING.has(token)) {
    const icon = token === "queued" || token.includes("queue") ? Loader2 : token === "active" ? PlayCircle : RefreshCw;
    return { label: prettyLabel(token), icon, variant: "info" };
  }
  if (STAGE_TODO.has(token)) return { label: prettyLabel(token), icon: Clock, variant: "default" };
...
// exact pill styles
const variantStyles: Record<VariantKey, string> = {
  info:       "border border-[#155DFC] text-[#155DFC] bg-[#155DFC]/10",
  brown:      "border border-[#6E4318CC] text-[#6E4318] bg-[#6E4318]/10",
  success:    "border border-[#09BF31] text-[#09BF31] bg-[#09BF31]/10",
  warning:    "border border-yellow-500 text-yellow-600 bg-yellow-50",
  destructive:"border border-red-500 text-red-600 bg-red-50",
  default:    "border border-neutral-300 text-neutral-700 bg-neutral-100",
};
...
const StatusBadge: React.FC<StatusBadgeProps> = ({
  ...
  return (
    <Badge
      // ignore badge's default variants; we fully style via className
      className={cn(
        "inline-flex items-center gap-1.5 rounded-full h-7 px-3 text-[12px] font-medium",
        "bg-white", // base so the tint looks clean
        variantStyles[finalVariant],
        className
      )}
    >
